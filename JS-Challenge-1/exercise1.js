class Cine {
    ...
    displayStatus() {
        return `Seat ${this.row}-${this.seatNumber}: ${this.isFree ? 'Free' : 'Occupied'}`;
    }
...
class Cinema {
    ...
    reserveSeat(row, seatNumber) {
        if (row < 1 || row > this.rows || seatNumber < 1 || seatNumber > this.seatPerRow) {
            console.log('Invalid seat selection');
            return;
        }

        const seat = this.seats.find(seat => seat.row === row && seat.seatNumber === seatNumber);
        if (seat) {
            seat.reserve();
        } else {
